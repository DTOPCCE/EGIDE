...
export default function App() {
    ...
    React.useEffect(() => {
        if (!firebaseConfig.apiKey) {
            setNotification({ message: "Configuração do Firebase não encontrada.", type: 'error' });
            setLoading(false);
            return;
        }
        const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
            if (firebaseUser) {
                ...
                } catch (error) { console.error("Erro na autenticação:", error); setNotification({ message: "Falha na autenticação. Verifique a configuração do Firebase.", type: 'error' }); }
            }
            setLoading(false);
        });
        return () => unsubscribe();
